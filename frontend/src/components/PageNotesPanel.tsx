...
export default function PageNotesPanel({ page, conversation, composer, onOpenSource }: Props) {
  const {
  ...
  if (mode === 'thread') {
    return <div className="flex h-full flex-col bg-[var(--lm-surface)]">
      ...
      <div className="min-h-0 flex-1"><ChatPanel messages={messages} onSendMessage={onSendFollowUp} onClearMessages={onClearMessages} isLoading={isLoading} token={token} pendingImages={pendingImages} onRemovePendingImage={onRemovePendingImage} onClearPendingImages={onClearPendingImages} error={error} thinkingStage={thinkingStage} thinkingStageKey={thinkingStageKey} isThinking={isThinking} compact externalContent={externalContent} onExternalContentConsumed={onExternalContentConsumed} externalFormula={externalFormula} onExternalFormulaConsumed={onExternalFormulaConsumed} externalFormulaQueue={externalFormulaQueue} onExternalFormulaQueueConsumed={onExternalFormulaQueueConsumed} onCancelGeneration={onCancelGeneration} onOpenPhoto={onOpenPhoto} onOpenSource={onOpenSource} /></div>
    </div>;
  }

  return <div className="flex h-full flex-col bg-[var(--lm-surface)]">
    <div className="hidden min-h-14 shrink-0 items-center border-b border-[var(--lm-border)] px-4 md:flex"><div><p className="text-sm font-semibold text-slate-700 dark:text-slate-100">本页概要</p><p className="mt-0.5 text-xs text-slate-400">第 {currentPage} 页 · {pageItems.length} 条记录</p></div></div>
