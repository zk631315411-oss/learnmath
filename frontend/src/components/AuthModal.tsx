...
const inputClass = "w-full border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-900 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-colors";
    ...
    <div className="fixed inset-0 bg-black/40 dark:bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-80 shadow-xl border border-slate-200/60 dark:border-slate-700/60">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-8 h-8 rounded-lg bg-indigo-100 dark:bg-indigo-950/50 flex items-center justify-center text-indigo-600 dark:text-indigo-300 text-sm">
            {mode === 'login' ? '→' : '+'}
          </div>
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <button type="submit" className="flex-1 px-4 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-200 dark:shadow-none">
              {mode === 'login' ? '登录' : '注册'}
            </button>
        ...
        <p className="mt-4 text-xs text-slate-400 dark:text-slate-500 text-center">
          ...
          <button onClick={onModeSwitch} className="text-indigo-600 dark:text-indigo-300 ml-1 hover:underline">{mode === 'login' ? '注册' : '登录'}</button>
        </p>
      </div>
    </div>
  );
