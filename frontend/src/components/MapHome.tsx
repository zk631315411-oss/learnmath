          ...
          <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-4">
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-slate-500 dark:text-slate-400">
                {firstCandidate && <><span>{firstCandidate.node.section} · {firstCandidate.node.name}</span><span aria-hidden="true">·</span></>}
                <span>{textbookName}</span>
                {firstCandidate && <><span aria-hidden="true">·</span><span>{statusLabel[firstCandidate.node.status]}</span></>}
              </div>
            </div>
            <div className="flex shrink-0 flex-wrap gap-2">
              {firstMarker && <button type="button" onClick={() => onOpenQuestion(firstMarker)} className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-300 px-4 text-sm font-medium text-slate-600 transition hover:border-slate-900 hover:text-slate-900 dark:border-slate-700 dark:text-slate-300 dark:hover:border-slate-300 dark:hover:text-slate-100"><MessageSquareText className="h-4 w-4" />来源提问</button>}
            ...
            return <li key={chapter.chapter}>
              ...
              {expanded && <div className="pb-4 pl-12 pr-4">
                {response?.sections.length ? <ul>{response.sections.map(section => {
                  ...
                  return <li key={section.section} className="flex items-baseline justify-between gap-4 border-t border-dashed border-slate-200 py-2 text-sm first:border-t-0 dark:border-slate-800"><span className="min-w-0 truncate text-slate-600 dark:text-slate-300">{section.section}</span><span className={`shrink-0 text-xs ${sectionReview ? 'text-rose-600 dark:text-rose-300' : 'text-slate-400'}`}>{sectionStatus}</span></li>;
