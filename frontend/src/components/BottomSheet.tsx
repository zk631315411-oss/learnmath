...
import { ChevronDown, ChevronLeft, ChevronRight, MessageCircle, PanelLeft, Scissors, X } from 'lucide-react';
  ...
  return (
    <>
      ...
      >
        ...
        <button type="button" onClick={() => controls?.goToPage(Math.max(1, currentPage - 1))} disabled={!controls || currentPage <= 1} className="icon-button shrink-0" title="上一页" aria-label="上一页"><ChevronLeft className="h-4 w-4" /></button>
        ...
        <button type="button" onClick={() => controls?.goToPage(Math.min(numPages, currentPage + 1))} disabled={!controls || !numPages || currentPage >= numPages} className="icon-button shrink-0" title="下一页" aria-label="下一页"><ChevronRight className="h-4 w-4" /></button>
        ...
        <button type="button" onClick={onCapture} disabled={interactionLocked} className="icon-button shrink-0 text-indigo-600 hover:bg-indigo-50 disabled:cursor-not-allowed disabled:opacity-50 dark:text-indigo-300 dark:hover:bg-indigo-950/50" title="框选页面内容" aria-label="框选"><Scissors className="h-4 w-4" /></button>
