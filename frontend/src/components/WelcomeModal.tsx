...
export default function WelcomeModal({ onClose, onDismiss, onOpenFeedback }: Props) {
  return (
    <div className="fixed inset-0 z-[10010] flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="welcome-title">
      <div className="relative max-h-[min(760px,calc(100dvh-32px))] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[var(--lm-border)] bg-[var(--lm-surface)] shadow-2xl">
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-[150px_minmax(0,1fr)] sm:p-7">
          <div className="flex items-center justify-center rounded-xl bg-[var(--lm-canvas)] p-4 sm:min-h-[180px]">
            <img src="/mascot/fox.png" alt="学数有道狐狸助手" className="max-h-40 w-auto object-contain" />
          </div>
          <div className="min-w-0 pr-5 sm:pr-2">
            <p className="mb-1 text-xs font-semibold tracking-wide text-[var(--lm-brand)]">学数有道内测</p>
            <h2 id="welcome-title" className="text-2xl font-bold text-[var(--lm-text-strong)]">欢迎来试用学数有道</h2>
            <p className="mt-2 text-sm leading-6 text-[var(--lm-text-muted)]">这是一个围绕教材、知识地图和 AI 答疑打造的学习工作台。你的真实使用反馈会直接帮助我们改进产品。</p>
            <div className="mt-5 space-y-3">
              {['沿教材章节定位知识点', '通过文字、截图和公式提问', '查看学习地图与学习记录'].map(item => (
                <div key={item} className="flex items-center gap-2 text-sm text-[var(--lm-text)]"><Check className="h-4 w-4 shrink-0 text-[var(--lm-success)]" />{item}</div>
              ))}
            </div>
            <div className="mt-5 rounded-xl border border-[var(--lm-border)] bg-[var(--lm-bg)] p-3 text-sm text-[var(--lm-text)]">
              <p className="font-semibold text-[var(--lm-text-strong)]">内测账号</p>
              <p className="mt-1 text-xs leading-5 text-[var(--lm-text-muted)]">可直接使用 <code className="rounded bg-[var(--lm-canvas)] px-1">test_001</code> 至 <code className="rounded bg-[var(--lm-canvas)] px-1">test_010</code>，统一密码为 <code className="rounded bg-[var(--lm-canvas)] px-1">123456</code>。也可以注册自己的账号。</p>
            </div>
            <div className="mt-5 rounded-xl border border-dashed border-[var(--lm-border)] p-3 text-sm text-[var(--lm-text-muted)]">
              <div className="flex items-center gap-2 font-medium text-[var(--lm-text)]"><ExternalLink className="h-4 w-4" />产品预览视频</div>
              <p className="mt-1 text-xs">视频入口正在准备中，当前可以直接开始体验核心功能。</p>
            </div>
          </div>
        </div>
