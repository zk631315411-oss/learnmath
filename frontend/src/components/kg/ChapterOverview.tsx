...
import { STATUS_LABEL, STATUS_ORDER, STATUS_VAR, sectionStatusSummary } from './shared';

/**
 * 章总览（地图模式首屏）：节列表 + 五态聚合条 + 小节状态摘要。
 * 不画图——图在点开小节后才出现。
 */
export default function ChapterOverview({ data, onOpenSection }: {
  data: NodeMapResponse;
  onOpenSection: (section: string) => void;
}) {
  return <div className="mx-auto w-full max-w-3xl px-4 py-4">
    <p className="mb-3 text-xs text-slate-500 dark:text-slate-400">点节行进入梯子，灰色是还没学的。</p>
    <div className="rounded-xl border border-[var(--lm-border)] bg-[var(--lm-surface)]">
      {data.sections.map((group, index) => {
        ...
        const summary = sectionStatusSummary(group.nodes);
        return <button
          key={group.section}
          type="button"
        ...
        >
          ...
          <span className="shrink-0 whitespace-nowrap text-xs text-slate-400">{group.nodes.length} 知识点 · {summary}</span>
