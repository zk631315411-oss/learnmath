...
前端是 LearnMath 的学生端交互层，把 PDF 教材变成"可提问的数学学习台"：选教材、翻页阅读，框选或输入题目后，AI 通过 SSE 流式返回解答；提问会以徽标形式钉在教材页面上，形成按页组织的提问记录，点击徽标可回看该题对话。移动端聊天入口收敛为 AiBall 浮球（含未读提醒），桌面端右栏常驻。
