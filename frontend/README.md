...
前端是 LearnMath 的学生端交互层，把 PDF 教材变成"可提问的数学学习台"：选教材、翻页阅读，框选或输入题目后，AI 通过 SSE 流式返回解答；提问会以徽标形式钉在教材页面上，形成按页组织的提问记录，点击徽标可回看该题对话。移动端聊天入口收敛为 AiBall 浮球（含未读提醒），桌面端右栏常驻；桌面端 PDF 左侧有按页提问记录侧栏（移动端为抽屉），点击条目跳页并载入该串对话。截图支持连发排队（待发最多 3 张、单删/可清空）；后端 `/solve-stream` 支持多图前每次提问只发送第一张，其余保留在待发列表。
