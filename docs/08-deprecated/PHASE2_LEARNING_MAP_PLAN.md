...
> **实施定案（2026-08-18）**：主回答只挂 `retrieve_kg_context`，不再包含自评提示或工具。回答文本流结束后，同一 SSE 请求使用主回答生成前的消息快照执行一次 one-shot 证据分叉，并强制调用 `report_turn_outcome`。快照锚定学生最近一条消息，不含教师针对该消息刚生成的回答与 thinking。完整设计、失败语义和验收记录见 `../07-decisions/EVIDENCE_FORK_DESIGN.md`。
...
- `textbook_id` **不进自报 schema**：由后端按当前问答上下文绑定（与 `retrieve_kg_context` 同一既定约定，见 `../07-decisions/KG_TOOL_DESIGN.md`）；落库时校验 node_id 前缀与上下文教材一致，不一致丢弃记日志。
