...
测试账号和密码必须从未纳入版本控制的本地环境变量读取：

- `LEARNMATH_TEST_USERNAME`
- `LEARNMATH_TEST_PASSWORD`

不得在文档、源码、自动化测试输出、截图文件名或提交日志中写入真实凭据。历史上曾写入仓库的测试密码应视为已经泄露并立即轮换。测试账号只用于 LearnMath 本地学生体验测试，不得用于生产环境、外部服务或保存真实个人学习数据。
...
6. 不用脚本直接插入 evidence；evidence 必须由真实 `/api/qa/solve-stream` 问答链路产生。
