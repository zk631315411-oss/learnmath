...
**审查边界**：仅静态代码核查，未做运行时 profiling，未修改任何文件。涉及性能与教材
形态的结论标注为「待验证」。
...
### 9. 流式渲染性能（先测量，再优化）
机制确认存在：每个 SSE chunk 重建 `messages` 数组（`useChat.ts:246-254`），
`ChatPanel` 的 memo 因引用变化失效，`MarkdownRenderer` 无 memo，历史消息的
Markdown/KaTeX 可能重复解析。**掉帧程度未实测**。步骤：长对话（50+ 消息含公式）
profiling → 若确认掉帧，做消息项级 memo（`MarkdownRenderer` 包 memo + 消息组件拆分）。
...
每个 Batch 单独提交；上一批测试不通过不得进入下一批。Batch 1–3 是本轮必须交付，Batch 4–5
可独立回滚，不得阻塞数据正确性和后台问答闭环。
...
| 长对话必然掉帧 | 存在机制性风险，掉帧程度待 profiling |

已解决：四本教材均为扫描版，本轮不启用文本层。仍待验证：流式性能实测（第 9 条）。
