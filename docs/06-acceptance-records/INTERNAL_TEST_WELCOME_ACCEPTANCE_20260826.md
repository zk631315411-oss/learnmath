...
| `npm run test:unit -- --run` | 7 files，40 tests passed |
| `npm run build` | TypeScript 与 Vite 生产构建通过 |
| `npx playwright test frontend-redesign.spec.ts` | 51 passed，53 条按项目/视觉开关预期 skipped，0 failed |
| `git diff --check` | 通过；仅有 Windows 行尾提示，无空白错误 |

E2E 明确覆盖：欢迎弹窗普通关闭与永久关闭、问卷必填和成功提交、桌面截图提问、历史对话、地图内联梯子、进度增量、重复 done 幂等，以及手机端四边吸附、350ms 长按、刷新持久化、上下边徽标和半屏避让。
...
检查结论：顶部停靠不侵入应用页眉，底部停靠不遮挡教材正文；工具栏始终向可用内容区展开，待处理徽标在四边均不被裁切。
