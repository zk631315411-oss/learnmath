...
> 日期：2026-08-20  
> 状态：待重新基线化，未经重新审阅不得执行。  
> 原因：静态目录、关系边和梯子视图已经实现，本草案的部分工作包已被覆盖。执行前必须逐项删除已完成内容，只保留真实缺口。  
> 关联文档：`../01-completed-plans/LEARNING_MAP_STATIC_CATALOG_PROGRESS_PLAN.md`、`../01-completed-plans/LEARNING_MAP_LADDER_VIEW_PLAN.md`、`../08-deprecated/MAP_PAGE_REDESIGN_PLAN.md`
